...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 21, 2025 at 05:56 PM
...
(12345, 'Lance Mendoza', 100100, '09956732919', 'Crochet Capybara Keychain For Lance'),
...
(1, 12345, 'Crochet Capybara Keychain For Lance', 'Accepted', '2025-11-21 16:35:19'),
(2, 12346, 'Crochet doll for Shaine', 'Accepted', '2025-11-21 16:35:31'),
(3, 12347, 'Crochet cat for Frtizie', 'Accepted', '2025-11-21 16:35:38');
...
,`ProjStat` enum('Accepted','In progress','Cancelled','Completed')
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_calendarview`  AS SELECT `g`.`DatePurchased` AS `DatePurchased`, `p`.`OrderLabel` AS `OrderLabel`, `p`.`ProjStat` AS `ProjStat` FROM (`tbl_gendetails` `g` join `tbl_progtrack` `p` on(`g`.`OrderID` = `p`.`OrderID`)) ORDER BY `g`.`DatePurchased` ASC ;
